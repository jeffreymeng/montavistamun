!(function (e, t) {
	...
	(function (e) {
		...
		function (e, t, r) {
			...
			const { MAILCHIMP_API_KEY: n } = process.env;
			r(17).default;
				...
				return new Promise(function (t, f) {
					...
					if (
						(d.open(
							...
							s(g, e.params, e.paramsSerializer),
							!0
						),
						(d.timeout = e.timeout),
			...
			e.exports = function (e) {
				return new Promise(function (t, r) {
					...
					if (
						(x["User-Agent"] ||
							x["user-agent"] ||
							(x["User-Agent"] = "axios/" + d.version),
						y && !n.isStream(y))
					) {
						if (Buffer.isBuffer(y));
						else if (n.isArrayBuffer(y))
							y = Buffer.from(new Uint8Array(y));
						else {
							if (!n.isString(y))
								return C(
									h(
					...
					b && delete x.Authorization;
					var O = g.test(_),
						F = O ? e.httpsAgent : e.httpAgent,
						A = {
							path: i(
								R.path,
								e.params,
					...
					e.socketPath
						...
						: ((A.hostname = R.hostname), (A.port = R.port));
					var j,
						S = e.proxy;
					if (!S && !1 !== S) {
						var B = _.slice(0, -1) + "_proxy",
							k = process.env[B] || process.env[B.toUpperCase()];
						if (k) {
							var T = p.parse(k),
								N =
							...
							if (
								q &&
								((S = { host: T.hostname, port: T.port }),
								T.auth)
							) {
								var L = T.auth.split(":");
								S.auth = { username: L[0], password: L[1] };
							}
						}
					}
					if (
