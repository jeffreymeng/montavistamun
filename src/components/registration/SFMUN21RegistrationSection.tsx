		...
		firebase
			...
			.then((snapshot) => {
				...
				if (!rawData?.personalInformation) {
					...
					!rawData.forms?.sfmun21DonationOptOut
				) {
					step = 4;
				} else {
					step = 5;
							...
							<VerticalSteps
								steps={[
									...
									},
									{
										title: "Donations",
										description:
											"To cover conference fees charged by SFMUN, we request a donation, which will go directly towards making this conference possible.",
									},
									{
										title: "You're Done!",
										description:
								...
								maxSwitchableStep={Math.min(
									!data.forms?.sfmun21FuhsdForm ? 2 : 1000,
									maxStep
								)}
								onStepSwitch={(i) => {
									if (stepHasChanges) {
