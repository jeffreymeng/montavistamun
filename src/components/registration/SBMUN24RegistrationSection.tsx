...
import ConfirmationSection from "./sbmun24sections/ConfirmationSection";
import DonationsSection from "./sbmun24sections/DonationsSection";
import PreferencesSection from "./sbmun24sections/PreferencesSection";
import WaiverFormsSection from "./sbmun24sections/WaiverFormsSection";
...
export default function SBMUN24RegistrationSection() {
	const name = "SBMUN 2024";
	const key = "sbmun24";
	useRequireLogin();
		...
		firebase
			...
			.then((snapshot) => {
				...
				if (!rawData?.personalInformation) {
					...
					step = 1;
				} else if (!rawData.forms?.sbmun24FuhsdForm) {
					step = 2;
				} else if (!rawData.preferences.sbmun24CommitteeConfirm) {
					step = 3;
				} else if (
					!rawData.forms?.sbmun24Donation &&
					!rawData.forms?.sbmun24DonationOptOut
				) {
					step = 4;
				} else {
					step = 5;
						...
						<div className="sticky px-4 sm:px-0 top-24">
							...
							<VerticalSteps
								...
								maxSwitchableStep={Math.min(
									!data.forms?.sbmun24FuhsdForm ? 2 : 1000,
									maxStep,
								)}
								onStepSwitch={(i) => {
									if (stepHasChanges) {
