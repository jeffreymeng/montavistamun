...
interface PreferencesInformation {
	...
	sfmun21CommitteeConfirm: boolean;
}
	...
	return (
		<RegisterFormSection<PreferencesInformation>
			...
			schema={Yup.object().shape({
				sfmun21Committee: Yup.array().required(),
				sfmun21CommitteeConfirm: Yup.boolean().isTrue(),
			})}
			loadingValues={{
				sfmun21Committee: ["Loading..."],
			}}
			confirmContinue={false}
		>
			{({
				values: { sfmun21Committee, sfmun21CommitteeConfirm },
				errors,
				canEdit,
				touched,
				setFieldValue,
				...
				return (
					<>
						<div className={"col-span-6"}>
							...
							<PreferenceList
								items={sfmun21Committee}
								setItems={(items) =>
									setFieldValue("sfmun21Committee", items)
								}
							/>
							<p className={"mt-1"}>
							...
							<div className="relative flex items-start mt-1">
								<div className="flex items-center h-5">
									<input
										id="committeeConfirm"
										checked={sfmun21CommitteeConfirm}
										onChange={(e) => {
											setFieldValue(
												"sfmun21CommitteeConfirm",
												e.target.checked
											);
										}}
										type="checkbox"
										className={
											...
											(errors.sfmun21CommitteeConfirm &&
											touched.sfmun21CommitteeConfirm
