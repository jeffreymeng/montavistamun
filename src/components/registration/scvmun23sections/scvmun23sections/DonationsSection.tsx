...
interface DonationForms {
	scvmun23Donation?: string;
	scvmun23DonationOptOut?: boolean;
}
interface ConfirmationData {
	scvmun23Confirmed: true;
}
	...
	return (
		<div className="mt-8 rounded-md shadow sm:overflow-hidden">
			<div className={"px-4 bg-white sm:p-6 py-4"}>
				...
				{data.forms?.scvmun23DonationOptOut && (
					<>
						<p className="mt-2">
			...
			<div className="flex justify-between px-4 py-3 text-right bg-gray-50 sm:px-6">
				...
				<button
					...
					onClick={() => {
						...
						if (data.confirm?.scvmun23Confirmed) {
							setStep(5);
							return;
						}
						setSubmitting(true);
						handleUpdateData("confirm", {
							...
							.then(() => {
								...
								return firebase
									...
									.doc(user?.uid)
									.update({
										scvmun23Registered: true,
										scvmun23RegistrationTimestamp: firebase.firestore.FieldValue.serverTimestamp(),
									});
							})
							.then(() => {
								setSubmitting(false);
				...
				>
					{uploading
						? "Uploading..."
						: (!receipt || receipt.length == 0) &&
						  !data?.forms?.scvmun3DonationOptOut
						? "Upload all forms to finish registration"
						: !data?.forms?.scvmun23Donation &&
						  !data?.forms?.scvmun23DonationOptOut
						? "Upload"
						: !data.confirm?.scvmun23Confirmed
						? "Finish Registration"
						: "Continue"}
				</button>
			</div>
