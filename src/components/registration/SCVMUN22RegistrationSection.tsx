...
import ConfirmationSection from "./scvmun22sections/ConfirmationSection";
import DonationsSection from "./scvmun22sections/DonationsSection";
import PreferencesSection from "./scvmun22sections/PreferencesSection";
import WaiverFormsSection from "./scvmun22sections/WaiverFormsSection";
...
export default function sfmun21RegistrationSection() {
	const name = "SCVMUN 2022";
	const key = "scvmun22";
	useRequireLogin();
			...
			.then((snapshot) => {
				...
				updateData({
					...
					preferences: {
						...
						scvmun22CommitteeConfirm: false,
				...
				if (!rawData?.personalInformation) {
					...
					step = 1;
				} else if (!rawData.forms?.scvmun22FuhsdForm) {
					step = 2;
				} else if (!rawData.preferences.scvmun22CommitteeConfirm) {
					step = 3;
				} else if (
					!rawData.forms?.scvmun22Donation &&
					!rawData.forms?.scvmun22DonationOptOut
				) {
					step = 4;
				} else {
					step = 5;
						...
						<div className="sticky px-4 sm:px-0 top-24">
							...
							<VerticalSteps
								...
								maxSwitchableStep={Math.min(
									!data.forms?.scvmun22FuhsdForm ? 2 : 1000,
									maxStep
								)}
								onStepSwitch={(i) => {
									if (stepHasChanges) {
