					...
					<div>
						...
						<Select
							...
							onChange={(o) => {
								...
								const conferenceData = data.find(
									(c) =>
										c.id === (o as { value: string }).value
								)?.award;
								setName(conferenceData?.name || "");
								const newAwards = ["", "", "", "", ""];
								conferenceData?.delegateAwards.forEach(
									(award) => {
										const index = [
											"Best Delegate",
											"Outstanding",
								...
								setEndDate(
									conferenceData?.time.toDate() || new Date()
								);
								setAwards(
									newAwards as [
										string,
								...
								setDelegationAward(
									conferenceData?.delegationAward || ""
								);
							}}
						/>
						<p className="mt-2 text-sm text-gray-500">
							To change the conference you are editing, you must
							first discard or save your changes.
						</p>
					</div>
					<form className={"mt-8"}>
